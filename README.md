...
- **Prebuilt-image friendly**: the whole install is one idempotent script
  (`iso-build/bootstrap-appliance.sh`), reused by a kickstart file and a
  Packer template.
...
See the main setup steps in the previous version of this README, or
`iso-build/README.md` for the one-shot bootstrap script / unattended ISO /
Packer options. In short:

```bash
sudo dnf install -y epel-release python3 python3-pip python3-venv certbot openssl
...
Or, for a one-command bootstrap on a fresh box:
```bash
...
2. **Firewalls** -- one entry per Palo Alto firewall, with API key (or
   username/password) and a "Test connection" button.
...
PAN-OS API call) and pick from real names via autocomplete, instead of
typing a profile name by hand and risking a typo that silently fails to
apply.
...
the common case, since a wildcard alone doesn't match the apex --  add
`example.com` under **Additional names (SANs)** on the domain form
(comma-separated for more than one). All names are issued together as one
certificate and deployed together to every listed target.

### Exporting/downloading a certificate

Once a certificate has been issued, a **Download cert** button appears on
its row in the Domains page. It bundles `fullchain.pem`, `cert.pem`,
`chain.pem`, and `privkey.pem` (whichever exist) into a zip -- useful when
a certificate (especially a wildcard) needs to be reused somewhere beyond
just the GlobalProtect deployment this appliance automates. Every
download is recorded in `/var/log/acme-appliance.log` (who, when, from
which IP) since it includes the private key.
...
now** (passes `--force-renewal` to certbot). Single-domain runs for
*different* domains can happen concurrently; re-running the same domain,
or running "all domains" while any single-domain renewal is in flight, is
blocked until the in-progress run finishes.

Secret fields are always masked in the UI and are only overwritten if you
type a new value -- leaving them blank on an edit keeps the existing
secret. The web UI writes to the exact same `appliance.yaml` that
`acme-renew.sh` reads, so hand-editing the YAML directly works too.
...
```
Prefer a dedicated API-only admin account with a role limited to
certificate import + the specific SSL/TLS profile/GlobalProtect xpath +
commit, rather than a full superuser account.
...
- Both a wildcard and its apex resolve to the **same**
  ...
  values at that name rather than overwriting one with the other (this
  was fixed specifically to support combined wildcard+apex issuance --
  Cloudflare's API naturally supports multiple records at one name
...
- On disk, `acme-renew.sh` passes an explicit `--cert-name` to certbot so
  ...
  shared by both the renewal script and the web UI so the two can never
  disagree on where a cert lives).
...
- **Logs**: `/var/log/acme-appliance.log`, viewable from the web UI's
  Logs page. certbot's own log: `/var/log/letsencrypt/letsencrypt.log`.
